...
public class PencilUpgradePanel : MonoBehaviour
{
    [SerializeField]
    ...
    {
        UpdateValues();
        rocketMovement = GameObject.Find("Rocket_Pencil").GetComponent<RocketMovement>();
        if (upgradeLevel == 0)
        {
    ...
    private void Update()
    {
        ...
        {
            rocketMovement.ChangePencilSprtie();
        }
    ...
    public void UpdateValues()
    {
        ...
        if (!rocket.Locked&&SpeedUpgradePanel.upgradeLevel>=25)
        {
    ...
    public void OnClickPurchase()
    {
        ...
        Rocket rocketInList = GameManager.Instance.CurrentUser.rocketPencilList.Find((x) => x.name == rocket.name);
        rocketInList.amount++;
        rocketInList.price = (long)(rocketInList.price * 1.07f + 2000);
        rocket.autoStar += 1500;
        rocket.autoStar *= 1.06f;
        UpdateUI();
        UpdateValues();
