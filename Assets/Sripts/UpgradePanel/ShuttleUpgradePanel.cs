...
public class ShuttleUpgradePanel : MonoBehaviour
{
    [SerializeField]
    ...
    {
        UpdateValues();
        rocketMovement = GameObject.Find("Rocket_Shuttle").GetComponent<RocketMovement>();
        if (upgradeLevel == 0)
        {
    ...
    private void Update()
    {
        ...
        {
            rocketMovement.ChangeShuttleSprtie();
        }
    ...
    public void UpdateValues()
    {
        ...
        if (!rocket.Locked&&FaceUpgradePanel.upgradeLevel>=25)
        {
    ...
    public void OnClickPurchase()
    {
        ...
        Rocket rocketInList = GameManager.Instance.CurrentUser.rocketShuttleList.Find((x) => x.name == rocket.name);
        rocketInList.amount++;
        rocketInList.price = (long)(rocketInList.price * 1.09f + 4500);
        rocket.autoStar += 3000;
        rocket.autoStar *= 1.08f;
        UpdateUI();
        UpdateValues();
