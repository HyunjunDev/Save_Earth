...
public class MoonUpgradePanel : MonoBehaviour
{
    [SerializeField]
    ...
    public static int moonUpgradeLevel = 0;
    public static float moonTouchStar = 0;
    ...
    {
        UpdateValues();
        imgaeChanger = GameObject.Find("Moon").GetComponent<ImgaeChanger>();
    }
    private void Update()
    {
        UpdateValues();
    ...
    {
        moonUpgradeLevel = planet.amount;
        switch (moonUpgradeLevel)
        {
            case 5:
                moonUpgradeLevel = planet.amount;
    ...
    public void UpdateValues()
    {
        ...
        if (!planet.Locked && MetorUpgradePanel.metorUpgradeLevel >= 70)
        {
            planetNameText.text = planet.name;
    ...
    public void OnClickPurchase()
    {
        ...
        Planet planetInList = GameManager.Instance.CurrentUser.moonList.Find((x) => x.name == planet.name);
        planetInList.amount++;
        planetInList.price = (long)(planetInList.price * 1.15f + 100000);
        planet.touchStar += 10000;
        planet.touchStar *= 1.1f;
        UpdateValues();
        PlanetUpdateUI();
