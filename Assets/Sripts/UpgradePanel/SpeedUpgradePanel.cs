...
public class SpeedUpgradePanel : MonoBehaviour
{
    [SerializeField]
    ...
    {
        UpdateValues();
        rocketMovement = GameObject.Find("Rocket_Speed").GetComponent<RocketMovement>();
        if (upgradeLevel == 0)
        {
    ...
    private void Update()
    {
        ...
        {
            rocketMovement.ChangeSpeedSprtie();
        }
    ...
    public void UpdateValues()
    {
        ...
        if (!rocket.Locked&&BananaUpgradePanel.upgradeLevel>=25)
        {
    ...
    public void OnClickPurchase()
    {
        ...
        Rocket rocketInList = GameManager.Instance.CurrentUser.rocketSpeedList.Find((x) => x.name == rocket.name);
        rocketInList.amount++;
        rocketInList.price = (long)(rocketInList.price * 1.06f + 1000);
        rocket.autoStar += 1000;
        rocket.autoStar *= 1.05f;
        UpdateUI();
        UpdateValues();
